<script setup>
    ...
    function handleSelectDate(date) {
        emit('toggle-calendar', formatDate(date))
    }
</script>
  ...
  <div class="month-calendar-container">
    <el-calendar v-model="value">
      <template #date-cell="{ data }">
        <!-- 日期容器 -->
        <div class="date-cell-content" @click="handleSelectDate(data.date)">
          <!-- 日期數字 -->
          <div 
            class="date-number" 
            :class="{ 
          ...
          <!-- 渲染完成/未完成項目 -->
          <div v-if="getTodosForDate(formatDate(data.date)).length > 0" class="todo-stats">
            <!-- 未完成項目 -->
            <div v-if="getIncompleteTodosForDate(formatDate(data.date)).length > 0" class="todo-stat-item incomplete">
              <span class="status-dot"></span>
              <span class="status-text">未完成: {{ getIncompleteTodosForDate(formatDate(data.date)).length }}</span>
            </div>
            <!-- 完成項目 -->
            <div v-if="getCompletedTodosForDate(formatDate(data.date)).length > 0" class="todo-stat-item completed">
              <span class="status-dot"></span>
